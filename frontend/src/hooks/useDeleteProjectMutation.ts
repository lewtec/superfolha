...
interface UseDeleteProjectMutationConfig {
  onCompleted?: (response: DeleteProjectMutationType['response'], errors: ReadonlyArray<Error> | null) => void;
  onError?: (error: Error) => void;
  updater?: DeleteProjectMutationType['updater'];
}

export function useDeleteProjectMutation(config?: UseDeleteProjectMutationConfig) {
  const [commit, isInFlight] = useMutation<DeleteProjectMutationType>(DeleteProjectMutationGraphql);
