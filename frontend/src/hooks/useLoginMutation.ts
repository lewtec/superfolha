...
interface UseLoginMutationConfig {
  onCompleted?: (response: LoginMutationType['response'], errors: ReadonlyArray<Error> | null) => void;
  onError?: (error: Error) => void;
}

export function useLoginMutation(config?: UseLoginMutationConfig) {
  const [commit, isInFlight] = useMutation<LoginMutationType>(LoginMutationGraphql);
