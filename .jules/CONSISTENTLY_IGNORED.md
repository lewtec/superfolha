...
**- Pattern:** Extracting interfaces (e.g., `ProjectReader`), moving logic to new files (e.g., `cookies.go`), or introducing arbitrary new packages without explicit user request (NOTE: centralized error reporting is an exception and IS explicitly mandated by global instructions).
